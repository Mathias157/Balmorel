...
#BSUB -o logs/GREAT_fullyear_2030_%J.out
#BSUB -e logs/GREAT_fullyear_2030_%J.err
...
if not [ -d "${PWD}/O2030/simex" ]; then
  mkdir O2030/simex
fi

# Copy or overwrite simex files from investment run, use /usr/bin/cp to avoid interactive mode defined in ~/.bashrc
echo "Copying simex files from ${investment_scenario} to O2030/simex"
/usr/bin/cp -rf $investment_scenario/simex/* O2030/simex/
