...
balmorel_colours["BIOGASMETHANATION"] = "#E8C3A8"
balmorel_colours["BIOMETH-DAC"] = "#E8C3A8"
...
@click.option(
    ...
    is_flag=True,
    required=False,
    help="Overwrite previous collected results?",
)
...
@click.option(
    ...
    required=False,
    default=".pdf",
    help='The extension of the plots, defaults to ".pdf"',
)
@click.option(
    "--path",
    type=str,
    required=False,
    default=".",
    help='Path to top level of Balmorel folders, defaults to "."',
)
@click.option(
    "--gams-sysdir",
    type=str,
...
@click.option(
    ...
    default=True,
    required=False,
    help="Plot generation capacities",
)
@click.option(
    "--sto",
    "-s",
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Include interpreted backup capacities from the @adequacy function in this plot",
)
@click.option(
    "--backup-nth-max",
    type=int,
    default=3,
    help="The nth-max value used for the @adequacy function, if backup capacities should be included",
)
@click.option(
    "--drop-hydro",
    is_flag=True,
    default=False,
    help="Include hydro-run-of-river in generation capacity plot?",
)
@click.option(
...
def cap(
    ...
    for key in [key for key in plot_types if plot_types[key]]:
        ...
        fig, ax = plt.subplots()
        if key == "generation":
            df = collect_results("G_CAP_YCRAF").query(
                'Technology != "H2-STORAGE" and not Technology.str.contains("INTERSEASONAL") and not Technology.str.contains("INTRASEASONAL") and not Generation.str.contains("BACKUP")'
            )
            ax.set_ylabel("Generation Capacity [GW]")
        else:
            df = collect_results("G_STO_YCRAF").query(
                'Technology == "H2-STORAGE" or Technology.str.contains("INTERSEASONAL") or Technology.str.contains("INTRASEASONAL")'
            )
...
def dem(ctx, commodity: str, filename: str = "demand"):
    ...
    df = collect_results(symbol[commodity.lower()])
...
def matrix(ctx, result: str):
    ...
    df = pd.DataFrame({"": df.values}, index=df.index).pivot_table(
        index="N", columns="M", values=""
    )
    df.index.name = ""
    df.columns.name = ""
...
@click.option(
    ...
    required=True,
    default=None,
    help="Groups of scenarios, with groups separated by ; and scenarios by ,",
)
@click.option(
    "--group-names",
    type=str,
...
def cost_change(ctx, sc_group: str, group_names: str, filename: str):
    ...
    for group in group_names:
        ...
        temp = df.pivot_table(
            index="Scenario", values="Value", aggfunc=lambda x: np.sum(x) / 1e3
        ).loc[scenarios]

        # Replace scenario names with resolution
        scenarios = (
            pd.Series(scenarios).str.replace("base", "N99").str.extract(r"([N]\d*)")[0]
        )
...
def electrolyser_vre_correlation(ctx, scenario_search_string):
    ...
    files = [
        file.name
        for file in path.glob(f"**/model/MainResults{scenario_search_string}.gdx")
    ]
    paths = [
        str(file.parent)
        for file in path.glob(f"**/model/MainResults{scenario_search_string}.gdx")
    ...
    for i, scenario in enumerate(files):
        ...
        df = df.query(
            '(Generation.str.contains("BACKUP") and Commodity == "ELECTRICITY") or Technology in ["ELECTROLYZER", "WIND-OFF", "WIND-ON", "SOLAR-PV", "SYNFUELPRODUCER"]'
        ).pivot_table(
            index=["Season", "Time"],
            columns="Technology",
            values="Value",
        ...
        print(
            (
                df["ELECTROLYZER"] * df[["WIND-OFF", "WIND-ON", "SOLAR-PV"]].sum(axis=1)
            ).sum()
            / (vre + elys)
        )
        ...
        print(
            (
                df["SYNFUELPRODUCER"]
                * df[["WIND-OFF", "WIND-ON", "SOLAR-PV"]].sum(axis=1)
            ).sum()
            / (vre + elys)
        )
...
def find_h2_synfuel_location(ctx, scenario: str, commodity: str, year: int = 2050):
    ...
    fig, ax = ctx.invoke(
        ...
        year=year,
        geofile=geofile,
        geofile_region_column=geofile_region_column,
...
def storage_profile(ctx, cluster: str, scenarios: str, size: str):
    ...
    for scenario in scenarios:
        ...
        fig.savefig(
            "%s/%s_storageprofile.pdf" % (ctx.obj["plot_path"], scenario),
            bbox_inches="tight",
        )
...
def sifnaios_profile(
    ctx, scenario: str, cluster: str, commodity: str, weather_year: int, freq: str
):
    """
    ...
    time_ind = pd.Series(
        pd.date_range(
            "%d-01-01-00:00" % weather_year, "%d-12-31-23:00" % weather_year, freq="1h"
        )
    )
    ...
    df = df.merge(index, on=["S", "T"], how="inner").pivot_table(
        index="time", columns=["result"], values="Value_ts-scaled", aggfunc="sum"
    )
    ...
    # Plot energy content
    ax.plot(
        storage_content.index + pd.Timedelta(weeks=2),
        storage_content,
        c=linecolor,
        linestyle="--",
    ...
    fig, ax = plot_style(
        fig,
        ax,
        "%s_sifstoprofile" % (scenario + "_" + cluster + "_" + commodity),
        legend=False,
    )
...
def vre_seas_prod(ctx, scenario: str):
    ...
    ax.legend(loc="lower center", ncol=3, bbox_to_anchor=(0.5, 1.03))
...
def allendofmodel(ctx, scenario: str, symbol: str):
    ...
    db = ws.add_database_from_gdx(
        os.path.abspath(
            os.path.join(ctx.obj["path"], scenario, "model", "all_endofmodel.gdx")
        )
    )
...
def get(ctx, symbol: str, pars, diff: bool):
    ...
    if type(filters) == str:
        df = df.query(filters).pivot_table(
            index="Scenario", columns=pars, values="Value", aggfunc="sum"
        )
    else:
        df = df.pivot_table(
            index="Scenario", columns=pars, values="Value", aggfunc="sum"
    ...
    if diff:
        ...
        for df_name in ["df_abs", "df_rel"]:
            ...
            with open("analysis/output/%s_output.html" % df_name, "w") as f:
                f.write(
                    transform(html)
                )  # Use transform to get inline styling, which is supported by Obsidian's markdown
    else:
        print(df.to_string())
        # Precision and general formatting
        html = (
...
def ptes_and_adequacy(ctx, scenario: str):
    ...
    # Get PTES areas
    df = res.get_result("G_STO_YCRAF").query(
        'Technology == "INTERSEASONAL-HEAT-STORAGE"'
    )
    ptes_caps = df.pivot_table(
        index="Area", columns="Generation", values="Value", aggfunc="sum"
    )
    ...
    ptes_caps["storage_share"] = (
        ptes_caps[ptes_caps.columns[:-1]].sum(axis=1) / ptes_caps["exo_heat_dem_GWh"]
    )
...
def plot_ptes_adequacy_relation(filter):
    ...
    fig, ax = plt.subplots()
    pt = df.pivot_table(
        index=["Scenario", "Area"], values=["LOLE_h", "storage_cap"], aggfunc="sum"
    )
    for i, scenario in enumerate(df.Scenario.unique()):
        color = next(color_cycle)
        marker = next(marker_cycle)
        pt.query("Scenario == @scenario").plot(
            ax=ax,
            kind="scatter",
            x="storage_cap",
            y="LOLE_h",
...
def RA_Plot(ctx, scenarios: str):
    ...
    for commodity in df.LOLE_h.columns:
        ax2.plot(
            ...
            marker="o",
            markeredgecolor="k",
            color=balmorel_colours[commodity],
            label=f"LOLE {commodity}",
        )
...
def bar_chart(ctx, symbol: str, index: str, columns: str, filename: str):
    ...
    Args:
        ...
        index (str): The index or indices to use for the pivot table.
        columns (str): The columns to use for the pivot table.
    """
...
def get_geofile(scenario, model_path):
    if re.match(r"N\d*M\d*", scenario):
        geofile = model_path.replace(
            "model", "data"
        ) + "/DE_%dcluster_geofile_2nd-order.gpkg" % (
            int(re.findall(r"M\d+", scenario)[0].lstrip("M"))
        )
        geofile_region_column = "cluster_name"
    elif re.match(r"N\d*", scenario):
        geofile = "analysis/geofiles/DE-DH-WNDFLH-SOLEFLH_%dcluster_geofile.gpkg" % (
            int(re.findall(r"N\d+", scenario)[0].lstrip("N"))
        )
        geofile_region_column = "cluster_name"
    else:
...
def sort_scenarios(df: pd.DataFrame):
    ...
    df["Scenario"] = pd.Categorical(
        ...
        categories=sorted(
            df["Scenario"].unique(),
            key=lambda x: (
                int(re.findall(r"N\d+", x)[0][1:])
                if re.findall(r"N\d+", x)
                else float("inf")
            ),
        ),
    )
    ...
    if os.path.exists(file_path) and not (ctx.obj["overwrite"]):
        ...
        for scenario in scenarios:
            if not (np.any(sto.Scenario.str.contains(scenario))):
                ...
                print(
                    "Could not find %s in existing storage profile files.." % scenario
                )
    else:
        file_exists = False
        scenario_in_existing_file = False
        ...
        for scenario in scenarios:
            for storage_type in ["inter", "intra"]:
                for carrier in ["electricity", "heat", "hydrogen"]:
                    ...
                    storage_results = get_storage_profiles(
                        os.path.join(
                            abspath,
                            m.scname_to_scfolder[scenario],
                            "model",
                            "all_endofmodel.gdx",
                        ),
                        carrier,
...
def load_geofile(scenario: str, cluster_params: str = "DE, DH, WNDFLH, SOLEFLH"):
    ...
    if "N" in scenario:
        geofile = gpd.read_file(
            "analysis/geofiles/%s_%dcluster_geofile.gpkg"
            % (cluster_params, int(scenario.replace("N", "")))
        )
    else:
        ...
        geofile.columns = pd.Series(geofile.columns).str.replace(
            "NAME_2", "cluster_name"
        )
