"""
Useful formats

Useful formats for data processing BALMOREL

Created on 27.04.2026
@author: Mathias Berg Rosendal
         PostDoc at DTU Management (Energy Economics & Modelling)
"""
# ------------------------------- #
#        0. Script Settings       #
# ------------------------------- #

import pandas as pd

# ------------------------------- #
#          1. Functions           #
# ------------------------------- #

testdf = pd.DataFrame(
    {
        "date": pd.date_range("2023-01-01", periods=5),
        "value": [10, 20, 30, 40, 50],
        "name": ["Freja", "Odin", "Ida", "Sven", "Bjørg"],
    }
)

# Should really not be hardcoded but loaded from scenario using symbol_to_df(db, "INDIVUSERS_AAA")
individual_heating_areas = [
    "BE_IDVU-HOTWTR",
    "DE4-E_IDVU-HOTWTR",
    "DE4-N_IDVU-HOTWTR",
    "DE4-S_IDVU-HOTWTR",
    "DE4-W_IDVU-HOTWTR",
    "DK1_IDVU-HOTWTR",
    "DK2_IDVU-HOTWTR",
    "EE_IDVU-HOTWTR",
    "FIN_IDVU-HOTWTR",
    "FR_IDVU-HOTWTR",
    "LT_IDVU-HOTWTR",
    "LV_IDVU-HOTWTR",
    "NL_IDVU-HOTWTR",
    "NO1_IDVU-HOTWTR",
    "NO2_IDVU-HOTWTR",
    "NO3_IDVU-HOTWTR",
    "NO4_IDVU-HOTWTR",
    "NO5_IDVU-HOTWTR",
    "PL_IDVU-HOTWTR",
    "SE1_IDVU-HOTWTR",
    "SE2_IDVU-HOTWTR",
    "SE3_IDVU-HOTWTR",
    "SE4_IDVU-HOTWTR",
    "UK_IDVU-HOTWTR",
    "BE_IDVU-SPACEHEAT",
    "DE4-E_IDVU-SPACEHEAT",
    "DE4-N_IDVU-SPACEHEAT",
    "DE4-S_IDVU-SPACEHEAT",
    "DE4-W_IDVU-SPACEHEAT",
    "DK1_IDVU-SPACEHEAT",
    "DK2_IDVU-SPACEHEAT",
    "EE_IDVU-SPACEHEAT",
    "FIN_IDVU-SPACEHEAT",
    "FR_IDVU-SPACEHEAT",
    "LT_IDVU-SPACEHEAT",
    "LV_IDVU-SPACEHEAT",
    "NL_IDVU-SPACEHEAT",
    "NO1_IDVU-SPACEHEAT",
    "NO2_IDVU-SPACEHEAT",
    "NO3_IDVU-SPACEHEAT",
    "NO4_IDVU-SPACEHEAT",
    "NO5_IDVU-SPACEHEAT",
    "PL_IDVU-SPACEHEAT",
    "SE1_IDVU-SPACEHEAT",
    "SE2_IDVU-SPACEHEAT",
    "SE3_IDVU-SPACEHEAT",
    "SE4_IDVU-SPACEHEAT",
    "UK_IDVU-SPACEHEAT",
    "IT_IDVU-HOTWTR",
    "IT_IDVU-SPACEHEAT",
    "CH_IDVU-HOTWTR",
    "CH_IDVU-SPACEHEAT",
    "AT_IDVU-HOTWTR",
    "AT_IDVU-SPACEHEAT",
    "CZ_IDVU-HOTWTR",
    "CZ_IDVU-SPACEHEAT",
    "ES_IDVU-HOTWTR",
    "ES_IDVU-SPACEHEAT",
    "PT_IDVU-HOTWTR",
    "PT_IDVU-SPACEHEAT",
    "SK_IDVU-HOTWTR",
    "SK_IDVU-SPACEHEAT",
    "HU_IDVU-HOTWTR",
    "HU_IDVU-SPACEHEAT",
    "SI_IDVU-HOTWTR",
    "SI_IDVU-SPACEHEAT",
    "HR_IDVU-HOTWTR",
    "HR_IDVU-SPACEHEAT",
    "RO_IDVU-HOTWTR",
    "RO_IDVU-SPACEHEAT",
    "BG_IDVU-HOTWTR",
    "BG_IDVU-SPACEHEAT",
    "GR_IDVU-HOTWTR",
    "GR_IDVU-SPACEHEAT",
    "IE_IDVU-HOTWTR",
    "IE_IDVU-SPACEHEAT",
    "LU_IDVU-HOTWTR",
    "LU_IDVU-SPACEHEAT",
    "AL_IDVU-HOTWTR",
    "AL_IDVU-SPACEHEAT",
    "ME_IDVU-HOTWTR",
    "ME_IDVU-SPACEHEAT",
    "MK_IDVU-HOTWTR",
    "MK_IDVU-SPACEHEAT",
    "BA_IDVU-HOTWTR",
    "BA_IDVU-SPACEHEAT",
    "RS_IDVU-HOTWTR",
    "RS_IDVU-SPACEHEAT",
    "TR_IDVU-HOTWTR",
    "TR_IDVU-SPACEHEAT",
    "MT_IDVU-HOTWTR",
    "MT_IDVU-SPACEHEAT",
    "CY_IDVU-HOTWTR",
    "CY_IDVU-SPACEHEAT",
]

# Should really not be hardcoded but loaded from scenario using symbol_to_df(db, "INDUSTRY_AAA")
industry_heating_areas = [
    "BE_IND-HT-NODH",
    "BE_IND-HT-DH",
    "DE4-E_IND-HT-NODH",
    "DE4-E_IND-HT-DH",
    "DE4-N_IND-HT-NODH",
    "DE4-N_IND-HT-DH",
    "DE4-S_IND-HT-NODH",
    "DE4-S_IND-HT-DH",
    "DE4-W_IND-HT-NODH",
    "DE4-W_IND-HT-DH",
    "DK1_IND-HT-NODH",
    "DK1_IND-HT-DH",
    "DK2_IND-HT-NODH",
    "DK2_IND-HT-DH",
    "EE_IND-HT-NODH",
    "EE_IND-HT-DH",
    "FIN_IND-HT-NODH",
    "FIN_IND-HT-DH",
    "FR_IND-HT-NODH",
    "FR_IND-HT-DH",
    "LT_IND-HT-NODH",
    "LT_IND-HT-DH",
    "LV_IND-HT-NODH",
    "LV_IND-HT-DH",
    "NL_IND-HT-NODH",
    "NL_IND-HT-DH",
    "NO1_IND-HT-NODH",
    "NO1_IND-HT-DH",
    "NO2_IND-HT-NODH",
    "NO2_IND-HT-DH",
    "NO3_IND-HT-NODH",
    "NO3_IND-HT-DH",
    "NO4_IND-HT-NODH",
    "NO4_IND-HT-DH",
    "NO5_IND-HT-NODH",
    "NO5_IND-HT-DH",
    "PL_IND-HT-NODH",
    "PL_IND-HT-DH",
    "SE1_IND-HT-NODH",
    "SE1_IND-HT-DH",
    "SE2_IND-HT-NODH",
    "SE2_IND-HT-DH",
    "SE3_IND-HT-NODH",
    "SE3_IND-HT-DH",
    "SE4_IND-HT-NODH",
    "SE4_IND-HT-DH",
    "UK_IND-HT-NODH",
    "UK_IND-HT-DH",
    "BE_IND-LT-NODH",
    "BE_IND-LT-DH",
    "DE4-E_IND-LT-NODH",
    "DE4-E_IND-LT-DH",
    "DE4-N_IND-LT-NODH",
    "DE4-N_IND-LT-DH",
    "DE4-S_IND-LT-NODH",
    "DE4-S_IND-LT-DH",
    "DE4-W_IND-LT-NODH",
    "DE4-W_IND-LT-DH",
    "DK1_IND-LT-NODH",
    "DK1_IND-LT-DH",
    "DK2_IND-LT-NODH",
    "DK2_IND-LT-DH",
    "EE_IND-LT-NODH",
    "EE_IND-LT-DH",
    "FIN_IND-LT-NODH",
    "FIN_IND-LT-DH",
    "FR_IND-LT-NODH",
    "FR_IND-LT-DH",
    "LT_IND-LT-NODH",
    "LT_IND-LT-DH",
    "LV_IND-LT-NODH",
    "LV_IND-LT-DH",
    "NL_IND-LT-NODH",
    "NL_IND-LT-DH",
    "NO1_IND-LT-NODH",
    "NO1_IND-LT-DH",
    "NO2_IND-LT-NODH",
    "NO2_IND-LT-DH",
    "NO3_IND-LT-NODH",
    "NO3_IND-LT-DH",
    "NO4_IND-LT-NODH",
    "NO4_IND-LT-DH",
    "NO5_IND-LT-NODH",
    "NO5_IND-LT-DH",
    "PL_IND-LT-NODH",
    "PL_IND-LT-DH",
    "SE1_IND-LT-NODH",
    "SE1_IND-LT-DH",
    "SE2_IND-LT-NODH",
    "SE2_IND-LT-DH",
    "SE3_IND-LT-NODH",
    "SE3_IND-LT-DH",
    "SE4_IND-LT-NODH",
    "SE4_IND-LT-DH",
    "UK_IND-LT-NODH",
    "UK_IND-LT-DH",
    "BE_IND-MT-NODH",
    "BE_IND-MT-DH",
    "DE4-E_IND-MT-NODH",
    "DE4-E_IND-MT-DH",
    "DE4-N_IND-MT-NODH",
    "DE4-N_IND-MT-DH",
    "DE4-S_IND-MT-NODH",
    "DE4-S_IND-MT-DH",
    "DE4-W_IND-MT-NODH",
    "DE4-W_IND-MT-DH",
    "DK1_IND-MT-NODH",
    "DK1_IND-MT-DH",
    "DK2_IND-MT-NODH",
    "DK2_IND-MT-DH",
    "EE_IND-MT-NODH",
    "EE_IND-MT-DH",
    "FIN_IND-MT-NODH",
    "FIN_IND-MT-DH",
    "FR_IND-MT-NODH",
    "FR_IND-MT-DH",
    "LT_IND-MT-NODH",
    "LT_IND-MT-DH",
    "LV_IND-MT-NODH",
    "LV_IND-MT-DH",
    "NL_IND-MT-NODH",
    "NL_IND-MT-DH",
    "NO1_IND-MT-NODH",
    "NO1_IND-MT-DH",
    "NO2_IND-MT-NODH",
    "NO2_IND-MT-DH",
    "NO3_IND-MT-NODH",
    "NO3_IND-MT-DH",
    "NO4_IND-MT-NODH",
    "NO4_IND-MT-DH",
    "NO5_IND-MT-NODH",
    "NO5_IND-MT-DH",
    "PL_IND-MT-NODH",
    "PL_IND-MT-DH",
    "SE1_IND-MT-NODH",
    "SE1_IND-MT-DH",
    "SE2_IND-MT-NODH",
    "SE2_IND-MT-DH",
    "SE3_IND-MT-NODH",
    "SE3_IND-MT-DH",
    "SE4_IND-MT-NODH",
    "SE4_IND-MT-DH",
    "UK_IND-MT-NODH",
    "UK_IND-MT-DH",
    "AT_IND-LT-NODH",
    "AT_IND-LT-DH",
    "AT_IND-MT-NODH",
    "AT_IND-MT-DH",
    "AT_IND-HT-NODH",
    "AT_IND-HT-DH",
    "CZ_IND-LT-NODH",
    "CZ_IND-LT-DH",
    "CZ_IND-MT-NODH",
    "CZ_IND-MT-DH",
    "CZ_IND-HT-NODH",
    "CZ_IND-HT-DH",
    "IT_IND-LT-NODH",
    "IT_IND-LT-DH",
    "IT_IND-MT-NODH",
    "IT_IND-MT-DH",
    "IT_IND-HT-NODH",
    "IT_IND-HT-DH",
    "CH_IND-LT-NODH",
    "CH_IND-LT-DH",
    "CH_IND-MT-NODH",
    "CH_IND-MT-DH",
    "CH_IND-HT-NODH",
    "CH_IND-HT-DH",
    "ES_IND-LT-NODH",
    "ES_IND-LT-DH",
    "ES_IND-MT-NODH",
    "ES_IND-MT-DH",
    "ES_IND-HT-NODH",
    "ES_IND-HT-DH",
    "PT_IND-LT-NODH",
    "PT_IND-LT-DH",
    "PT_IND-MT-NODH",
    "PT_IND-MT-DH",
    "PT_IND-HT-NODH",
    "PT_IND-HT-DH",
    "SK_IND-LT-NODH",
    "SK_IND-LT-DH",
    "SK_IND-MT-NODH",
    "SK_IND-MT-DH",
    "SK_IND-HT-NODH",
    "SK_IND-HT-DH",
    "HU_IND-LT-NODH",
    "HU_IND-LT-DH",
    "HU_IND-MT-NODH",
    "HU_IND-MT-DH",
    "HU_IND-HT-NODH",
    "HU_IND-HT-DH",
    "SI_IND-LT-NODH",
    "SI_IND-LT-DH",
    "SI_IND-MT-NODH",
    "SI_IND-MT-DH",
    "SI_IND-HT-NODH",
    "SI_IND-HT-DH",
    "HR_IND-LT-NODH",
    "HR_IND-LT-DH",
    "HR_IND-MT-NODH",
    "HR_IND-MT-DH",
    "HR_IND-HT-NODH",
    "HR_IND-HT-DH",
    "RO_IND-LT-NODH",
    "RO_IND-LT-DH",
    "RO_IND-MT-NODH",
    "RO_IND-MT-DH",
    "RO_IND-HT-NODH",
    "RO_IND-HT-DH",
    "BG_IND-LT-NODH",
    "BG_IND-LT-DH",
    "BG_IND-MT-NODH",
    "BG_IND-MT-DH",
    "BG_IND-HT-NODH",
    "BG_IND-HT-DH",
    "GR_IND-LT-NODH",
    "GR_IND-LT-DH",
    "GR_IND-MT-NODH",
    "GR_IND-MT-DH",
    "GR_IND-HT-NODH",
    "GR_IND-HT-DH",
    "IE_IND-LT-NODH",
    "IE_IND-LT-DH",
    "IE_IND-MT-NODH",
    "IE_IND-MT-DH",
    "IE_IND-HT-NODH",
    "IE_IND-HT-DH",
    "LU_IND-LT-NODH",
    "LU_IND-LT-DH",
    "LU_IND-MT-NODH",
    "LU_IND-MT-DH",
    "LU_IND-HT-NODH",
    "LU_IND-HT-DH",
    "AL_IND-LT-NODH",
    "AL_IND-LT-DH",
    "AL_IND-MT-NODH",
    "AL_IND-MT-DH",
    "AL_IND-HT-NODH",
    "AL_IND-HT-DH",
    "ME_IND-LT-NODH",
    "ME_IND-LT-DH",
    "ME_IND-MT-NODH",
    "ME_IND-MT-DH",
    "ME_IND-HT-NODH",
    "ME_IND-HT-DH",
    "MK_IND-LT-NODH",
    "MK_IND-LT-DH",
    "MK_IND-MT-NODH",
    "MK_IND-MT-DH",
    "MK_IND-HT-NODH",
    "MK_IND-HT-DH",
    "BA_IND-LT-NODH",
    "BA_IND-LT-DH",
    "BA_IND-MT-NODH",
    "BA_IND-MT-DH",
    "BA_IND-HT-NODH",
    "BA_IND-HT-DH",
    "RS_IND-LT-NODH",
    "RS_IND-LT-DH",
    "RS_IND-MT-NODH",
    "RS_IND-MT-DH",
    "RS_IND-HT-NODH",
    "RS_IND-HT-DH",
    "TR_IND-LT-NODH",
    "TR_IND-LT-DH",
    "TR_IND-MT-NODH",
    "TR_IND-MT-DH",
    "TR_IND-HT-NODH",
    "TR_IND-HT-DH",
    "MT_IND-LT-NODH",
    "MT_IND-LT-DH",
    "MT_IND-MT-NODH",
    "MT_IND-MT-DH",
    "MT_IND-HT-NODH",
    "MT_IND-HT-DH",
    "CY_IND-LT-NODH",
    "CY_IND-LT-DH",
    "CY_IND-MT-NODH",
    "CY_IND-MT-DH",
    "CY_IND-HT-NODH",
    "CY_IND-HT-DH",
]

# ------------------------------- #
#            2. Main              #
# ------------------------------- #
