...
def fueloil():
    ...
    try:
        result = load_row(
            "backcast/historicalprices/Weekly_Oil_Bulletin_Prices_History_maticni_4web.xlsx",
            1077,
            "Prices wo taxes",
            [0, 2],
        )
...
def naturalgas():
    ...
    try:
        ...
        try:
            df = pd.read_csv(
                "backcast/historicalprices/natural_gas_TTF_2024.csv",
                on_bad_lines="skip",
            )
        except TypeError:
            df = pd.read_csv(
...
def coal():
    ...
    try:
        df = pd.read_csv(
            "backcast/historicalprices/Rotterdam Coal Futures Historical Data.csv"
        )
